...
function syncTransferFormAvailability() {
  const walletReady = Boolean(contract && signer);
  const opsEnabled = contractOperationsEnabled;
  const on = walletReady && opsEnabled;
  ...
  document.getElementById('btn-transfer-via-proxy').disabled = !on || !proxyContract;
}
...
function renderTransferPreview() {
  ...
  const breakdown = computeTransferBreakdown(amountEth);
  if (!breakdown) {
    el.hidden = true;
    el.innerHTML = '';
    return;
...
async function refreshHeaderBalances() {
  ...
  await Promise.all([showContractBalance(), showWalletBalance()]);
}
...
document.getElementById('transfer-form').addEventListener('submit', transferEther);
document
