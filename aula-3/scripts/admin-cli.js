/**
 * CLI interativo para operacoes administrativas do EtherTransfer e SimpleToken (onlyOwner).
 *
...
const MENU_ACTIONS = [
  { name: '📊 Status — EtherTransfer + SimpleToken', value: 'status' },
  new inquirer.Separator('— EtherTransfer —'),
  ...
  new inquirer.Separator('— SimpleToken (AULA) —'),
  { name: '🪙 mint — distribuir tokens para uma carteira', value: 'tokenMint' },
  { name: '🔥 burnFrom — queimar tokens de uma carteira', value: 'tokenBurnFrom' },
  { name: '💱 setEthRate — cotacao do token em relacao ao ETH', value: 'tokenSetRate' },
  { name: '📈 tokenStatus — supply, cotacao e saldo de carteira', value: 'tokenStatus' },
...
async function connectAdminContracts() {
  ...
  if (!tokenAddress) {
    throw new Error('Defina TOKEN_ADDRESS no .env (npm run deploy:local).');
  }

  const wallet = await getSignerFromEnv();
  const contract = await ethers.getContractAt('EtherTransfer', contractAddress, wallet);
  const token = await ethers.getContractAt('SimpleToken', tokenAddress, wallet);

  return { contract, token, wallet, contractAddress, tokenAddress, rpcUrl };
}

async function showStatus(ctx) {
  const { contract, token, wallet, contractAddress, tokenAddress, rpcUrl } = ctx;
  const [owner, enabled, balance, registered, tokenOwner, totalSupply, weiPerToken, tokenName, tokenSymbol] =
    await Promise.all([
      contract.owner(),
      contract.enabled(),
  ...
  console.log('EtherTransfer:    ', contractAddress);
  console.log('SimpleToken:      ', tokenAddress);
  console.log('Sua carteira:     ', wallet.address);
  console.log('Owner EtherTransfer:', owner);
  console.log('Owner SimpleToken:  ', tokenOwner);
  console.log(
    'Voce e o owner?   ',
  ...
  console.log('Operacoes ETH ativas?', enabled ? 'sim (enabled)' : 'nao (suspenso)');
  console.log('Saldo contrato ETH: ', ethers.formatEther(balance), 'ETH');
  ...
  console.log(
    'Cotacao:          ',
    `1 ${tokenSymbol} = ${ethers.formatEther(weiPerToken)} ETH (referencia)`
  );
  console.log('----------------\n');
}
...
async function promptTokenAmount(message) {
...
async function runTokenMint(token) {
  const to = await promptWalletAddress('Carteira que recebera os tokens:');
  const amountHuman = await promptTokenAmount('Quantidade de tokens (ex.: 100):');
  const amount = ethers.parseEther(amountHuman);
  const tx = await token.mint(to, amount);
  await waitTx(`mint(${to}, ${amountHuman})`, tx);
}

async function runTokenBurnFrom(token) {
  const from = await promptWalletAddress('Carteira de onde queimar tokens:');
  const amountHuman = await promptTokenAmount('Quantidade a queimar:');
  const amount = ethers.parseEther(amountHuman);
  const tx = await token.burnFrom(from, amount);
  await waitTx(`burnFrom(${from}, ${amountHuman})`, tx);
}

async function runTokenSetRate(token) {
  const { rateEth } = await inquirer.prompt([
    {
      type: 'input',
      name: 'rateEth',
      message: 'Valor de 1 token inteiro em ETH (ex.: 0.001):',
      validate: (input) => {
        const n = Number(input.trim());
        return Number.isFinite(n) && n > 0 ? true : 'Informe um numero maior que zero';
      },
  ...
  const weiPerToken = ethers.parseEther(rateEth.trim());
  const tx = await token.setEthRate(weiPerToken);
  await waitTx(`setEthRate(${rateEth.trim()} ETH por token)`, tx);
}

async function runTokenStatus(token, wallet) {
  const [name, symbol, totalSupply, weiPerToken] = await Promise.all([
    token.name(),
    token.symbol(),
    token.totalSupply(),
    token.weiPerToken(),
  ...
  const { wallet: walletInput } = await inquirer.prompt([
    {
      type: 'input',
      name: 'wallet',
      message: 'Consultar saldo de qual carteira? (vazio = sua carteira)',
      default: '',
    },
  ]);

  const address = walletInput.trim() || wallet.address;
  const balance = await token.balanceOf(address);

  console.log('\n--- SimpleToken ---');
  console.log('Nome / simbolo: ', `${name} (${symbol})`);
  console.log('Total supply:   ', ethers.formatEther(totalSupply), symbol);
  console.log(
    'Cotacao:        ',
    `1 ${symbol} = ${ethers.formatEther(weiPerToken)} ETH (referencia)`
  );
  console.log('Saldo carteira: ', ethers.formatEther(balance), symbol);
  console.log('-----------------\n');
}
...
async function handleAction(action, ctx) {
  switch (action) {
    ...
    case 'clearAll':
      ...
      break;
    case 'tokenMint':
      await runTokenMint(ctx.token);
      break;
    case 'tokenBurnFrom':
      await runTokenBurnFrom(ctx.token);
      break;
    case 'tokenSetRate':
      await runTokenSetRate(ctx.token);
      break;
    case 'tokenStatus':
      await runTokenStatus(ctx.token, ctx.wallet);
      break;
    case 'exit':
      return false;
    default:
...
async function runMenu() {
  ...
  try {
    ctx = await connectAdminContracts();
    console.log('\nEtherTransfer + SimpleToken — CLI administrativo (onlyOwner)\n');
    await showStatus(ctx);
